                                ...
                                Right (header, source) ->
                                  case unitName opts of
                                   Nothing -> do putStrLn "/* START HEADER */"
                                                 ...
                                                 putStrLn source
                                   Just name -> do writeFile (name ++ ".h") (wrapHeader opts header)
                                                   writeFile (name ++ ".c") (addIncludes opts name source)
...
doConvert opts mparsed = do
  ...
  case parsed of
   ...
   Right exprs -> (fmap (fmap concat)) $ sequence <$> (forM exprs $ \expr ->
