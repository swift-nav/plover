...
writeFiles (header, source) imports opts unitName =
  ...
  case unitName of
   Nothing -> do putStrLn "/* START HEADER */"
                 ...
                 return Nothing
   Just name -> do
     let headerPrefix = "/* File Generated by PLOVER */\n"
     ...
     writeFile hfile $ headerPrefix ++ (wrapHeader (map addPrefix normalIncludes) name header)
     writeFile cfile $ headerPrefix ++ (addIncludes (map addPrefix staticIncludes) includeName source)
